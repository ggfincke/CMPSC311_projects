...
int mdadm_mount(void) 
{
  ...
  if (is_mounted == 1)
  ...
  else
    {
      ...
      //set mounted to True
      is_mounted = 1;
      //create cache (set to 4096)
      cache_create(4096);
      return 1;
    }
}

//unmount, now destroys cache
...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf)
{
  ...
  if (start_addr > 0x100000 || read_len > 0x800 || is_mounted == 0 || (read_buf == NULL && read_len != 0) || (read_len + start_addr) > 0x100000)
      ...
      else
	{
	  ...
	  //for the cache: added if/else statements to each case to determine if in cache using cache lookup; if not in cache, it gets added, if it is, it is read from the cache 
	  while (completed < read_len)
	    {
	      ...
	      if (byteInBlock != 0)
		{
		  //if disk and block is not in cache
		  if (cache_lookup(diskID, blockID, temp_buf) == -1)
		    {
		      //continue as normal, but insert jbod for diskID and blockID into cache 
		...
		{
		  //if not in cache then proceed normally + insert into cache
		  if (cache_lookup(diskID, blockID, temp_buf) == -1)
		    {
		      jbod_operation(readBlock, temp_buf);
		      cache_insert(diskID, blockID, temp_buf);
...
//write with cache implemented
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf)
{
  //write from *write_buf into write_len starting at start_addr
  ...
  if (start_addr > 0x100000 || write_len > 0x800 || is_mounted == 0 || is_written == 0 || (write_buf == NULL && write_len != 0) || (write_len + start_addr) > 0x100000)
      ...
      else
	{
	  ...
	  //while completed bytes is less than the read length
	  //cache is implemented here and with the mdadm_read; implemented by using cache_update to update the cache values as well as JBOD_write to directly modify JBOD 
	  while (completed < write_len)
	    {
	      //reading block into temp_buf and seeking back to the expected block (bc read increments by 1)
	      //mdadm_read should use cache and insert the element into it, if it is not already 
	      ...
	      if (byteInBlock != 0)
		{
		  ...
		  //update cache with temp_buf (temp_buf contains the original read + anything new)
		  cache_update(diskID, blockID, temp_buf);
